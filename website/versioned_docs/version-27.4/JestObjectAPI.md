...
### `elric.mocked<T>(item: T, deep = false)`

The `mocked` test helper provides typings on your mocked modules and even their deep methods, based on the typing of its source. It makes use of the latest TypeScript feature, so you even have argument types completion in the IDE (as opposed to `elric.MockInstance`).
...
const mockedFoo = elric.mocked(foo, true);
...
test('direct', () => {
  ...
  expect(elric.mocked(foo.name).mock.calls).toHaveLength(1);
});
```
